...
--cuda-kernel NAME         CUDA kernel 变体：standard、dual、fixed-npt1、fixed-npt2、fixed-npt4、lop3、fixed-lop3-npt1、fixed-lop3-npt2 或 fixed-lop3-npt4
...
`cuda.kernel` 支持 `standard`、`dual`、`fixed-npt1`、`fixed-npt2`、`fixed-npt4`、`lop3`、`fixed-lop3-npt1`、`fixed-lop3-npt2` 或 `fixed-lop3-npt4`。默认使用 `standard`；`--cuda-autotune` 会测试可用变体，并为当前 GPU 保留最快的稳定结果。fixed-npt kernel 会在运行时强制使用对应的 `nonces-per-thread` 值，`lop3` 会用 inline PTX 三输入逻辑指令实现 SHA256 的 `CH` 和 `MAJ`，fixed-lop3 会组合这两种选择。
