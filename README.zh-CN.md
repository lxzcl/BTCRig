...
- 可选 CUDA GPU 路径，使用 NVIDIA Driver API 和内嵌 PTX，包含 standard、dual nonce、fixed-npt 和 lop3 kernel 变体；运行时默认关闭，目标机器不需要安装 CUDA Toolkit。
...
--cuda-kernel NAME         CUDA kernel 变体：standard、dual、fixed-npt1、fixed-npt2、fixed-npt4 或 lop3
...
`cuda.kernel` 支持 `standard`、`dual`、`fixed-npt1`、`fixed-npt2`、`fixed-npt4` 或 `lop3`。默认使用 `standard`；`--cuda-autotune` 会测试可用变体，并为当前 GPU 保留最快的稳定结果。fixed-npt kernel 会在运行时强制使用对应的 `nonces-per-thread` 值，`lop3` 会用 inline PTX 三输入逻辑指令实现 SHA256 的 `CH` 和 `MAJ`。
