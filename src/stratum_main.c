...
static int run_opencl_self_test(const miner_opencl_config_t *config) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        device_config.backend_variant = devices[i].backend_variant;
...
static int run_cuda_self_test(const miner_cuda_config_t *config) {
#if defined(BTC_MINER_CUDA)
    ...
    if (cuda_miner_self_test(config, &result, error, sizeof(error)) != 0) {
        fprintf(stderr,
                "%s[CUDA]%s self-test failed: %s\n",
                C_BRIGHT_RED,
                C_RESET,
                error[0] != '\0' ? error : "unknown error");
        return 1;
    }
    printf("%s[CUDA]%s self-test ok device=%s%s%s compute=sm_%d%d driver=%d nonces=%u\n",
           C_BRIGHT_GREEN,
           C_RESET,
           C_BRIGHT_CYAN,
           result.device_name,
    ...
    (void)config;
    fprintf(stderr,
            "%s[CUDA]%s self-test unavailable: this build was compiled without CUDA support\n",
            C_BRIGHT_RED,
            C_RESET);
    return 1;
#endif
...
static int parse_opencl_backend_variant(const char *value, int fallback) {
    ...
    if (string_equals_ci(value, "compat10") || string_equals_ci(value, "compat")) {
        return MINER_OPENCL_BACKEND_COMPAT10;
    }
    if (string_equals_ci(value, "modern")) {
...
static int parse_cuda_kernel_variant(const char *value, int fallback) {
    ...
    if (string_equals_ci(value, "lop3") ||
        string_equals_ci(value, "standard-lop3") ||
...
#if defined(BTC_MINER_CUDA)
    miner_cuda_config_defaults(&config->cuda);
#else
    memset(&config->cuda, 0, sizeof(config->cuda));
#endif
...
static int load_config_file(app_config_t *config, const char *path, int required) {
    ...
    if (json_is_object(opencl)) {
        ...
        config->opencl.backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(opencl, "backend")),
                                                                      config->opencl.backend_variant);
        config->opencl.backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(opencl, "backend-variant")),
                                                                      config->opencl.backend_variant);
        config->opencl.backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(opencl, "backend_variant")),
                                                                      config->opencl.backend_variant);
        ...
        if (json_is_array(devices)) {
            ...
            json_array_foreach(devices, index, device) {
                ...
                dst->backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(device, "backend")),
                                                                    config->opencl.backend_variant);
                dst->backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(device, "backend-variant")),
                                                                    dst->backend_variant);
                dst->backend_variant = parse_opencl_backend_variant(json_string_value(json_object_get(device, "backend_variant")),
                                                                    dst->backend_variant);
    ...
    json_t *cuda = json_object_get(root, "cuda");
    if (json_is_object(cuda)) {
        config->cuda.enabled = json_bool_value(json_object_get(cuda, "enabled"), config->cuda.enabled);
        config->cuda.device = json_int_value(json_object_get(cuda, "device"), config->cuda.device);
        config->cuda.batch_size = json_u32_value(json_object_get(cuda, "batch-size"), config->cuda.batch_size);
        config->cuda.batch_size = json_u32_value(json_object_get(cuda, "batch_size"), config->cuda.batch_size);
        config->cuda.threads_per_block = json_u32_value(json_object_get(cuda, "threads-per-block"),
                                                        config->cuda.threads_per_block);
        config->cuda.threads_per_block = json_u32_value(json_object_get(cuda, "threads_per_block"),
                                                        config->cuda.threads_per_block);
        config->cuda.threads_per_block = json_u32_value(json_object_get(cuda, "block"),
                                                        config->cuda.threads_per_block);
        config->cuda.nonces_per_thread = json_u32_value(json_object_get(cuda, "nonces-per-thread"),
                                                        config->cuda.nonces_per_thread);
        config->cuda.nonces_per_thread = json_u32_value(json_object_get(cuda, "nonces_per_thread"),
                                                        config->cuda.nonces_per_thread);
        config->cuda.nonces_per_thread = json_u32_value(json_object_get(cuda, "npt"),
                                                        config->cuda.nonces_per_thread);
...
static void autotune_disable_cuda(miner_cuda_config_t *config) {
    if (config == NULL) {
        return;
    }
#if defined(BTC_MINER_CUDA)
    miner_cuda_config_defaults(config);
#else
    memset(config, 0, sizeof(*config));
#endif
    config->enabled = 0;
...
static double autotune_run_mode(int cpu_threads,
    ...
    miner_t *miner = miner_create_with_backend_options(cpu_threads, opencl, cuda);
    if (miner == NULL) {
        return 0.0;
    }
    ...
    if (cuda != NULL && cuda->enabled && miner_thread_count(miner) <= cpu_threads) {
        miner_destroy(miner);
        return 0.0;
    }
...
static int autotune_batch_hashrate_is_better(double candidate, double best) {
...
static void autotune_opencl_device_params(const miner_opencl_config_t *base,
    ...
    int requested_backend = device->backend_variant != MINER_OPENCL_BACKEND_AUTO ?
        device->backend_variant : base->backend_variant;
    if (requested_backend == MINER_OPENCL_BACKEND_COMPAT10) {
        backend_candidates[backend_count++] = MINER_OPENCL_BACKEND_COMPAT10;
    } else if (requested_backend == MINER_OPENCL_BACKEND_MODERN) {
        backend_candidates[backend_count++] = MINER_OPENCL_BACKEND_MODERN;
    } else {
        backend_candidates[backend_count++] = MINER_OPENCL_BACKEND_COMPAT10;
        backend_candidates[backend_count++] = MINER_OPENCL_BACKEND_MODERN;
    }
    ...
    printf("%s[AUTOTUNE]%s tuning gpu%d OpenCL backend/kernel/local-work-size/npi\n",
           C_CYAN,
           C_RESET,
           device_index);
    ...
    for (int ki = 0; ki < kernel_count; ++ki) {
        ...
        for (int li = 0; li < local_count; ++li) {
            ...
            printf("%s[AUTOTUNE]%s testing gpu%d backend=%s kernel=%s batch=%u local=%u npi=%u\n",
                   C_CYAN,
                   C_RESET,
                   device_index,
                   opencl_backend_variant_name(candidate.backend_variant),
            ...
            if (ok) {
                ...
                printf("%s[AUTOTUNE]%s gpu%d backend=%s kernel=%s batch=%u local=%u npi=%u unavailable\n",
                       C_YELLOW,
                       C_RESET,
                       device_index,
                       opencl_backend_variant_name(candidate.backend_variant),
    ...
    if (best_ok) {
        *device = best_device;
        printf("%s[AUTOTUNE]%s gpu%d selected backend=%s kernel=%s batch=%u local=%u npi=%u hashrate=%.3f MH/s\n",
               C_BRIGHT_GREEN,
               C_RESET,
               device_index,
               opencl_backend_variant_name(device->backend_variant),
    ...
    printf("%s[AUTOTUNE]%s gpu%d final backend=%s kernel=%s batch=%u local=%u npi=%u hashrate=%.3f MH/s\n",
           C_BRIGHT_GREEN,
           C_RESET,
           device_index,
           opencl_backend_variant_name(device->backend_variant),
...
static const char *autotune_cuda_mode_name(const miner_cuda_config_t *cuda) {
...
static int save_autotune_config(const char *path,
    ...
    json_t *cuda = json_object_get_or_create(root, "cuda");
    json_t *autotune = json_object_get_or_create(root, "autotune");
    if (cpu == NULL || opencl == NULL || cuda == NULL || autotune == NULL) {
        json_decref(root);
        return -1;
    }
    ...
    json_object_set_new(opencl, "backend", json_string(opencl_backend_variant_name(config->opencl.backend_variant)));
    ...
    if (config->opencl.enabled && config->opencl.device_count > 0) {
        ...
        for (int i = 0; i < config->opencl.device_count; ++i) {
            ...
            json_object_set_new(item, "backend", json_string(opencl_backend_variant_name(device->backend_variant)));
    ...
    json_object_set_new(cuda, "enabled", config->cuda.enabled ? json_true() : json_false());
    json_object_set_new(cuda, "device", json_integer(config->cuda.device));
        ...
        if (result->opencl.device_count > 0) {
            ...
            if (devices != NULL) {
                for (int j = 0; j < result->opencl.device_count; ++j) {
                    ...
                    json_object_set_new(device_item, "backend", json_string(opencl_backend_variant_name(device->backend_variant)));
...
static int run_autotune(app_config_t *config, const char *config_path) {
    ...
    miner_cuda_config_t cpu_only_cuda;
    ...
    int tune_cuda = config->cuda.enabled;

    printf("%s[AUTOTUNE]%s first-run benchmark seconds=%.1f strategy=%s\n",
           C_CYAN,
           C_RESET,
           seconds,
           (tune_opencl || tune_cuda) ?
               (full_threads > 0 ? "cpu,gpu,cpu+gpu" : "gpu-only") : "cpu-only");
...
#if defined(BTC_MINER_CUDA)
    if (!tune_cuda) {
        printf("%s[AUTOTUNE]%s CUDA skipped: disabled in config; set cuda.enabled=true or use --cuda to benchmark NVIDIA GPU modes\n",
               C_YELLOW,
               C_RESET);
    } else {
...
#else
    if (tune_cuda) {
        printf("%s[AUTOTUNE]%s CUDA skipped: this build was compiled without CUDA support\n", C_YELLOW, C_RESET);
    }
#endif
...
static int app_config_autotune_required(const app_config_t *config) {
    ...
    if ((config->opencl.enabled || config->cuda.enabled) && !config->gpu_autotune_done) {
...
static void usage(const char *argv0) {
    ...
    printf("  %s --cuda-self-test [--cuda-device N]\n", argv0);
    printf("  %s --cpu-info\n", argv0);
    printf("  %s --version\n", argv0);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--self-test") == 0) {
            ...
            if (parsed < 0) {
                fprintf(stderr, "%s[CONFIG]%s invalid --opencl-backend, use auto, compat10, or modern\n",
                        C_BRIGHT_RED,
                        C_RESET);
                return 2;
            }
            app_config.opencl.backend_variant = parsed;
            worker_override = 1;
    ...
    if (cuda_self_test) {
        return run_cuda_self_test(&app_config.cuda);
    }
