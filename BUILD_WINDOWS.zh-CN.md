...
Windows 构建可以包含 CPU、OpenCL 和 CUDA 后端。OpenCL 和 CUDA 都通过运行时驱动加载，目标机器缺少 GPU runtime 时也不影响 CPU-only 启动。
...
# 可选 OpenCL 构建支持。安装 headers 后 CMake 会编译 GPU 后端。
pacman -S --needed \
...
```text
OpenCL 支持是构建时可选模块。如果要让 CMake 编译 OpenCL worker，
需要安装 OpenCL 开发头文件。OpenCL runtime 只会在启用 OpenCL 时加载，
所以缺少 OpenCL.dll 不影响 CPU-only 启动。CUDA 通过运行时驱动加载编译进程序，只有设置
