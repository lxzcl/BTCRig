...
- 可选 OpenCL GPU 路径，包含 compat10 兜底和 OpenCL 1.2+ modern 候选选择，运行时默认关闭。
...
| `opencl` | 可选 `btc_stratum` worker | OpenCL GPU worker，包含 `compat10` 兜底和 `modern` OpenCL 1.2+ 候选路径 |
